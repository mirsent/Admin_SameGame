 ...
 * @return Array
 */
function getDateFromRange($enddate)
{

    $stimestamp = strtotime(date("Y-m-d"));
...
function getHoliday($date){
    ...
    $params = array(
          "key" => "349dce0632de7004b48bcdbce331e9fd",
          "year-month" => date('Y-n'),
    );
    $paramstring = http_build_query($params);
    $content = juhecurl($url,$paramstring);
    $result = json_decode($content,true);
    $holiday = $result['result']['data']['holiday'];
    $holidays = [];
    $workdays = [];
    foreach (json_decode($holiday, true) as $value) {
        $list = $value['list'];
        foreach ($list as $v) {
            $status = $v['status'];
            $date = $v['date'];
            if ($status == 1) array_push($holidays, $date); // 假期
            else if ($status == 2) array_push($workdays, $date); // 补假
        }
    }
    return [
        "holidays" => $holidays,
...
function is_in_array(&$date,$holidays,$workdays){
    $week = date("w",strtotime($date));
    if (in_array(date('Y-n-j', strtotime($date)),$holidays)) {
        $date = date("Y-n-j",strtotime("-1 days",strtotime($date)));
        is_in_array($date,$holidays,$workdays);
    } else if ( $week == 0 || $week == 6 && !in_array(date('Y-n-j', strtotime($date)),$workdays) ) {
        $date = date("Y-n-j",strtotime("-1 days",strtotime($date)));
        is_in_array($date,$holidays,$workdays);
    } else {
        return $date;
